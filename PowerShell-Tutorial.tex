...
The single most important thing to internalize before you write any
PowerShell: \textbf{PowerShell pipes objects, not text.}
...
\cmd{Get-Member} is the secret weapon: \emph{pipe anything to it} and
PowerShell tells you exactly which properties and methods that object
exposes. This is how you discover, for example, that the result of
...
This is the section that pays for the whole tutorial. If you only learn
five PowerShell cmdlets, learn these:
...
\cmd{Measure-Object} does numeric aggregation:
...
PowerShell strings come in four useful forms. Pick the right one to
avoid the most common Windows footgun (mangled escapes).
...
The single most common Windows footgun: \cmd{Set-Content} and
...
This is where PowerShell starts to feel genuinely good for data work.
\cmd{Import-Csv} reads a CSV and yields \textbf{one object per row},
with column names as properties. No manual splitting, no quoting bugs,
...
\vspace{1em}
\noindent\textit{One last thing:} when you're stuck, the answer almost
always starts with \cmd{Get-Command} \texttt{*word*} (find the
cmdlet), \cmd{Get-Help} \texttt{NAME -Examples} (see how to use
it), and \cmd{Get-Member} (see what properties an object has).
PowerShell rewards discovery in a way bash never did.
