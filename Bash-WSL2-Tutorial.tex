...
\texttt{sort} and \texttt{uniq} aggregate. Master this composition
pattern and you can solve any data-shaping problem on the command line.
...
The single most important WSL2 lesson: \textbf{keep your code under
...
correctly; your file paths are POSIX; everything Just Works.
...
\subsection*{Tab completion is the killer feature}
...
This is the section that pays for the whole tutorial. The four
fundamental ideas:
...
\subsection*{Quoting: the one rule you must internalize}
...
The single most useful family of bash features. \texttt{\$\{VAR\}}
extends to a rich mini-language:
...
\texttt{\$\{\#arr[@]\}} (not \texttt{len(arr)}), and -- crucially
...
The five tools that turn bash into a data engineer's playground:
\cmd{grep}, \cmd{sed}, \cmd{awk}, \cmd{sort}, \cmd{uniq}. Learn
them in this order, in roughly this depth.
...
\subsection*{\texttt{awk} -- the data engineer's secret weapon}

\texttt{awk} treats each line as a record split into fields. The
default field separator is whitespace; \texttt{-F,} sets it to a
comma.
...
That's a complete data pipeline in five lines, with no Python, no
dependencies, no parsing libraries -- and it runs in milliseconds
on a million-line log.
...
\texttt{csvsql} is killer for ad-hoc analysis: full SQLite SQL, run
against any CSV, no schema definition needed.
...
\texttt{jq} is to JSON what \texttt{awk} is to text: small,
composable, ubiquitous. Every modern API speaks JSON; every modern
config file is JSON or YAML. \texttt{jq} is how you read and
transform them from the shell.
...
\begin{compareps}
\texttt{jq} is the shell-native equivalent of PowerShell's
\pscmd{ConvertFrom-Json} / \pscmd{ConvertTo-Json} -- with one big
upgrade: it has no \texttt{-Depth} footgun. \texttt{jq} preserves
arbitrary nesting by default. The trade-off is that \texttt{jq}'s
filter language is its own DSL, with a learning curve.
\end{compareps}

% =========================================================
\section{HTTP and APIs with \texttt{curl}}

\texttt{curl} is the swiss-army HTTP client. It's installed
everywhere, supports every protocol you'll need, and pipes
straight into \texttt{jq}.
...
Python's \texttt{httpx} or \texttt{aiohttp}; for ad-hoc API
exploration, \texttt{curl + jq} is hard to beat.
\end{comparepy}
...
Docker on WSL2 is the easiest Linux Docker setup that exists. Two
paths to install it:
...
trade-offs: a slightly less seamless experience (you start the
...
Windows GPU for training. The single most important rule: \textbf{do
not install a Linux NVIDIA driver inside Ubuntu.} The GPU is
shared from Windows; the only driver that should exist is the
Windows one.
...
\subsection*{The discipline four: \texttt{set -euo pipefail}}
...
The list of bash footguns every newcomer hits exactly once. Read it
once, recognize them next time.
...
\vspace{1em}
\noindent
\textbf{Last advice.} When you're stuck, the answer almost always
starts with one of three commands: \cmd{man cmd} (the manual page),
\cmd{cmd --help} (the short help), or \cmd{type cmd} (is it a
built-in, an alias, or a binary on \texttt{PATH}?). Bash rewards a
small set of well-learned tools over a large set of half-known
ones -- pick five from this tutorial (\texttt{grep}, \texttt{awk},
\texttt{jq}, \texttt{curl}, \texttt{uv}) and use them every day for
two weeks. You'll be fluent.
